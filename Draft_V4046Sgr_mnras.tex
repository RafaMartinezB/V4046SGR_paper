...
\begin{abstract}
  The nearby V4046\,Sgr spectroscopic binary hosts a gas-rich disc known for its wide cavity and dusty ring.  We present high resolution ($\sim$20--50\,mas **** WRONG UPDATE TO CORRECT NUMBERS****) ALMA observations of the 1.3\,mm  continuum which, combined with SPHERE-IRDIS polarized images and a well-sampled spectral energy distribution (SED), allow us to propose a physical model for the source using radiative transfer (RT) predictions. The new ALMA data reveal a very thin ring at a radius of 13.32$\pm$0.26\,au (Ring13), with a marginally resolved radial width of 1.50$\pm$0.72\,au. Ring13 is surrounded by a $\sim$10\,au-wide gap, and it is flanked by a very mm-bright outer ring (Ring24) with a sharp inner edge at 24\,au. The intensity plateau in Ring24 breaks at $\sim$35\,au, where it steeply drops  and  extends into  a shallow tail. A compact source of mm-continuum at  the stellar position is picked up by ALMA, with a radius of ***RADIUS OF THE BLOB, IS IT RESOLVED?***.  In order to reproduce the SED in the IR, our RT model  requires an  inner circumbinary disc ***GIVE RADII*** rich in small grains (as in previous  models of V4046\,Sgr), and  which  matches the  central ALMA signal if it contains **Xxx*** Mearths in large dust. An inner  ring at $\sim$6\,au (Ring6) composed of small  grains, with a gas mass of ***Mearth*** and deprived of large dust, is required to explain the ***10$\mu$m ???*** flux density. The surprisingly thin Ring13 is nonetheless $\sim$5 times wider ****NOT CONSISTENT WITH CONCLUSIONS*** in the ALMA signal  than the model scale height, and $\sim$3***???*** narrower than the model gas radial width. Ring13 could thus be   long-lived and shaped by radial dust trapping, which would require a turbulence value of  $\alpha_{\rm turb} \sim 0.01$.   The strong near-far disc asymmetry at 1.65\,$\micron$ points at a very forward-scattering phase function, and requires grain radii of no less than 0.4\,$\micron$. The previously reported scattered-light shadow of the secondary star is also reproduced by the RADMC3D RT package. 
\end{abstract}
...
The effective beam size of the presented ALMA observations marginally resolves the width of Ring13 to be $w_\mathrm{d} = 1.50\mathrm{au}$. Using that the FWHM of Ring13 in the   RT model for the gas  corresponds to $w_\mathrm{g} = 5.32\mathrm{au}$, we can see that large dust grains have to be radially converged to explain the discrepancy.

Even though the radial spread of large dust grains in Ring13 appears to be quite thin, the width in comparison to the sub-lying gas profile speaks for the presence of considerable turbulent diffusion. Following a similar ring analysis as in \citet{2018ApJ...869L..46D}, we find that the ratio between the dimensionless diffusion parameter, $D$, and the dimensionless Stokes number, St ****GIVE FORMULA USED HERE****, (which parameterizes the dynamical behaviour of a grain) is roughly, $D/{\rm St}\approx 0.1$. The observed signal is expected to be dominated by grains of size $a\approx 0.02\,{\rm cm}$, the RT model prescribes a gas density of $\Sigma_{\rm g} \approx 0.5\,{\rm g}{\rm cm}^{-2}$ to the location of Ring13. With these values, the relevant Stokes number is approximated to be St$\approx 0.1$. This yields an estimate for the level of diffusivity of $D\approx 0.01$. It further provides a value for the level of turbulence in Ring13, $\alpha_{\rm turb}\approx 0.01$, assuming turbulence to be equal to the level of diffusion \citep{2007Icar..192..588Y}. We note that the obtained value for $\alpha_{\rm turb}$ is larger than typical expectations \citep[e.g.][for V4046 Sgr]{Flaherty_2020} *****QUOTE THE EXPECTATIOMS???i think that theirs is an upper limit****. Our estimate  carries uncertainties originating from estimates of the local gas density, which varies proportionally to the dust-to-gas mass ratio $\zeta$, 


**** as well as the fact that the width of Ring13 is measured at different vertical positions for small and large grains in the observations <<< I WOULD REMOVE THIS.

The observed SED is compared with the model in Fig.~\ref{fig:SED}. From the similarity with the data we propose that there has to be a small-grain population close to the stars down to 0.3\,au ****SO IN LINE WITH PREVIOUS RT MODELS, CITE THEM***. The decision of employing a three ringed structure for the gas and  small dust grains relies on the fact that the SED needed a ring at a radius smaller than 10\,au to have a proper fit around 10\,$\micron$, which we place here at 6\,au (Ring6). This ring  is intriguing  as it is deprived of millimetre-size dust grains to explain its absence in the ALMA image (***upper limit Zeta in this ring?? use 3 times noise value in the ALMA image***), while  a thin ring like the one we implemented would be expected to trap millimetre-size dust grains.
...
\begin{enumerate}
\item We report the detection of a radio counterpart to the inner ring observed in  scattered-light images. This ring is much narrower  in the 1.3\,mm continuum, with a radial width of 1.50$\pm$0.72\,au at a radius of 13.32$\pm$0.26\,au. According to our RT model, this ring contains 0.9\,M$_{\earth}$ in millimetre-sized grains. Its width in the dust is still 3.1 times its estimated height  ($H= $ 0.48\,au at 13.32\,au), and **** times narrower than the gas, which make it a case of strong radial trapping in a long-lived ring. 

 
  
  \item The 1.3\,mm outer ring, that starts at $\sim$24\,au and has its peak intensity at $\sim$30\,au, presents a visible break in the surface brightness at $\sim$36\,au.  ***HOW IS THIS BREAK INTERPRETED IN THE RT**** ??
  ...
  \item Our parametric model, which accounts for the SED of the system, involves the existence of a sub-micron dust population close (<5\,au) to the stars. ***IN LINE WITH PREVIOUS WORK**** except for ring6, comment on this and explain it. 
